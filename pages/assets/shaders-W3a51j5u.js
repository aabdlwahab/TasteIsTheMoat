...
`;function t(e){return Object.entries(e).map(([e,t])=>`uniform ${t.type===`color`?`vec3`:t.type} ${e};`).join(`
...
`,`
`,n].join(`
`)}function r(e){return n(e,``).split(`
`).length-1}function i(e,t,n){let r=e.createShader(t);if(!r)return`Failed to create shader object.`;if(e.shaderSource(r,n),e.compileShader(r),!e.getShaderParameter(r,e.COMPILE_STATUS)){let t=e.getShaderInfoLog(r)??`Unknown shader error.`;return e.deleteShader(r),t.replace(/\0/g,``).trim()}return r}function a(e,t,n){let r=i(e,e.VERTEX_SHADER,t);if(typeof r==`string`)return r;let a=i(e,e.FRAGMENT_SHADER,n);if(typeof a==`string`)return e.deleteShader(r),a;let o=e.createProgram();if(!o)return e.deleteShader(r),e.deleteShader(a),`Failed to create shader program.`;if(e.attachShader(o,r),e.attachShader(o,a),e.linkProgram(o),e.deleteShader(r),e.deleteShader(a),!e.getProgramParameter(o,e.LINK_STATUS)){let t=e.getProgramInfoLog(o)??`Program link failed.`;return e.deleteProgram(o),t.replace(/\0/g,``).trim()}return o}function o(e,t,n,r){switch(n.type){case`float`:e.uniform1f(t,r);break;case`vec2`:{let n=r;e.uniform2f(t,n[0],n[1]);break}case`color`:{let n=r;e.uniform3f(t,n[0],n[1],n[2]);break}}}function s(e,t){return e.replace(/ERROR:\s*(\d+):(\d+):/g,(e,n,r)=>`ERROR: ${n}:${Math.max(1,Number(r)-t)}:`)}function c(e){"@babel/helpers - typeof";return c=typeof Symbol==`function`&&typeof Symbol.iterator==`symbol`?function(e){return typeof e}:function(e){return e&&typeof Symbol==`function`&&e.constructor===Symbol&&e!==Symbol.prototype?`symbol`:typeof e},c(e)}function l(e,t){if(c(e)!=`object`||!e)return e;var n=e[Symbol.toPrimitive];if(n!==void 0){var r=n.call(e,t||`default`);if(c(r)!=`object`)return r;throw TypeError(`@@toPrimitive must return a primitive value.`)}return(t===`string`?String:Number)(e)}function u(e){var t=l(e,`string`);return c(t)==`symbol`?t:t+``}function ee(e,t,n){return(t=u(t))in e?Object.defineProperty(e,t,{value:n,enumerable:!0,configurable:!0,writable:!0}):e[t]=n,e}var te={id:`aurora`,name:`Aurora`,description:`Glowing northern-lights ribbons over a night sky.`,category:`gradient`,colorRoles:{u_colorA:`bright`,u_colorB:`accent`,u_skyTop:`dark`,u_skyBottom:`dark`},uniforms:{u_colorA:{type:`color`,value:[.12,.95,.62],label:`Ribbon low`},u_colorB:{type:`color`,value:[.3,.35,.98],label:`Ribbon high`},u_skyTop:{type:`color`,value:[.02,.03,.09],label:`Sky top`},u_skyBottom:{type:`color`,value:[.04,.06,.12],label:`Sky bottom`},u_speed:{type:`float`,value:.5,min:0,max:2,label:`Speed`},u_intensity:{type:`float`,value:.9,min:0,max:2,label:`Intensity`},u_grain:{type:`float`,value:.03,min:0,max:.2,label:`Grain`}},fragment:`
void main() {
...
`},oe={id:`holo-foil`,name:`Iridescent Foil`,description:`Crinkled foil under real spectral thin-film interference.`,category:`iridescent`,interactive:!0,uniforms:{u_thickness:{type:`float`,value:520,min:150,max:1200,label:`Film nm`},u_ior:{type:`float`,value:1.62,min:1.1,max:2.8,label:`Film IOR`},u_scale:{type:`float`,value:1.05,min:.6,max:8,label:`Crinkle scale`},u_relief:{type:`float`,value:.44,min:.02,max:1,label:`Relief`},u_lightHeight:{type:`float`,value:.9,min:0,max:3,label:`Light height`},u_gloss:{type:`float`,value:.65,min:0,max:1,label:`Gloss`},u_gain:{type:`float`,value:3.1,min:0,max:5,label:`Gain`},u_saturation:{type:`float`,value:1,min:0,max:1.4,label:`Saturation`},u_speed:{type:`float`,value:1,min:0,max:2,label:`Motion speed`},u_pointer:{type:`float`,value:1,min:0,max:2.5,label:`Pointer force`},u_grain:{type:`float`,value:.035,min:0,max:.2,label:`Grain`}},fragment:`
...
`},m={id:`metaballs`,name:`Metaballs`,description:`Gooey blobs that merge and split. Playful, tactile.`,category:`organic`,colorRoles:{u_bg:`dark`,u_colorA:`mid`,u_colorB:`accent`},uniforms:{u_bg:{type:`color`,value:[.04,.05,.1],label:`Background`},u_colorA:{type:`color`,value:[.98,.3,.45],label:`Core`},u_colorB:{type:`color`,value:[.99,.75,.3],label:`Edge`},u_speed:{type:`float`,value:.6,min:0,max:2,label:`Speed`},u_spread:{type:`float`,value:.9,min:.2,max:1.6,label:`Spread`},u_threshold:{type:`float`,value:1,min:.4,max:2,label:`Threshold`},u_grain:{type:`float`,value:.02,min:0,max:.2,label:`Grain`}},fragment:`
void main() {
  vec2 uv = gl_FragCoord.xy / u_resolution.xy;
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
...
`},h=[[.025,.035,.1],[.42,.12,.95],[.04,.88,.78]];function g(e){let{colors:t=h,...n}=e;return{...n,category:`gradient`,uniforms:{u_colorA:{type:`color`,value:t[0],label:`Base`},u_colorB:{type:`color`,value:t[1],label:`Color`},u_colorC:{type:`color`,value:t[2],label:`Accent`},u_speed:{type:`float`,value:.4,min:0,max:2,step:.01,label:`Speed`},u_scale:{type:`float`,value:1.8,min:.3,max:6,step:.01,label:`Scale`},u_softness:{type:`float`,value:1,min:.2,max:2.5,step:.01,label:`Softness`},u_intensity:{type:`float`,value:1,min:0,max:2,step:.01,label:`Intensity`}}}}var _=[g({id:`gradient-drift`,name:`Gradient Drift`,description:`Slow mesh-like colour fields drifting past one another.`,colors:[[.03,.04,.12],[.48,.12,.95],[.02,.78,.95]],fragment:`
void main() {
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
...
}`}),g({id:`aurora-bloom`,name:`Aurora Bloom`,description:`Wide luminous ribbons blooming through a midnight gradient.`,colors:[[.015,.025,.09],[.15,.22,.95],[.12,.95,.68]],fragment:`
void main() {
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),g({id:`fluid-spectrum`,name:`Fluid Spectrum`,description:`Liquid spectral colour transported through smooth domain warping.`,colors:[[.1,.015,.18],[.92,.1,.48],[.04,.85,.94]],fragment:`
void main() {
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),g({id:`diagonal-tide`,name:`Diagonal Tide`,description:`Broad diagonal bands sweeping across a softened colour field.`,colors:[[.04,.035,.13],[.96,.25,.5],[.98,.66,.18]],fragment:`
void main() {
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),g({id:`radial-bloom`,name:`Radial Bloom`,description:`Overlapping radial colour blooms orbiting around the canvas.`,colors:[[.025,.02,.09],[.38,.18,.98],[.96,.18,.6]],fragment:`
void main() {
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),g({id:`chromatic-current`,name:`Chromatic Current`,description:`A directional colour current with slowly curling eddies.`,colors:[[.015,.055,.11],[.02,.72,.75],[.68,.2,.96]],fragment:`
void main() {
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),g({id:`soft-orbs`,name:`Soft Orbs`,description:`Large blurred colour orbs floating through a dark gradient.`,colors:[[.025,.03,.08],[.22,.35,.98],[.92,.2,.62]],fragment:`
void main() {
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
  vec3 color = u_colorA;
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),g({id:`sunset-waves`,name:`Sunset Waves`,description:`Warm horizon colours rolling in layered horizontal waves.`,colors:[[.055,.025,.13],[.96,.2,.4],[1,.62,.2]],fragment:`
void main() {
  vec2 uv = gl_FragCoord.xy / u_resolution.xy;
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
...
void main() {
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),S({id:`deterministic-duotone-flow`,name:`Deterministic Duotone Flow`,description:`Seeded high-contrast currents flowing between two tonal fields.`,seed:31,colors:[[.015,.025,.08],[.08,.32,.96],[.02,.92,.76],[.82,.98,.9]],fragment:`
void main() {
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),S({id:`palette-morph`,name:`Palette Morph`,description:`The complete gradient palette cycles smoothly through four colours.`,seed:4,colors:[[.04,.025,.12],[.92,.16,.46],[.1,.76,.96],[.98,.72,.22]],fragment:`
void main() {
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),S({id:`lit-mesh-waves`,name:`Lit Mesh Waves`,description:`A displaced mesh gradient shaded with moving highlights.`,seed:12,colors:[[.025,.035,.1],[.32,.12,.92],[.02,.74,.82],[.9,.96,1]],fragment:`
...
void main() {
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),S({id:`water-plane-gradient`,name:`Water Plane Gradient`,description:`A reflective gradient horizon rolling across a perspective water plane.`,seed:23,colors:[[.015,.035,.09],[.08,.22,.72],[.02,.74,.84],[.98,.58,.32]],fragment:`
void main() {
  vec2 uv = gl_FragCoord.xy / u_resolution.xy;
  vec2 p = uv * 2.0 - 1.0;
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),S({id:`frosted-wave-stack`,name:`Frosted Wave Stack`,description:`Translucent gradient waves layered behind softly textured glass.`,seed:9,colors:[[.025,.035,.09],[.28,.34,.96],[.04,.86,.78],[.84,.92,1]],fragment:`
void main() {
  vec2 uv = gl_FragCoord.xy / u_resolution.xy;
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),S({id:`liquid-metal-gradient`,name:`Liquid Metal Gradient`,description:`Flowing colour bands polished with chrome-like reflections.`,seed:42,colors:[[.015,.02,.055],[.22,.1,.72],[.02,.78,.82],[.96,.9,1]],fragment:`
...
void main() {
  ...
  color += u_colorD * specular * u_intensity;
  gl_FragColor = vec4(color, 1.0);
}`}),S({id:`layered-radial-sweep`,name:`Layered Radial Sweep`,description:`Overlapping radial gradients expand, rotate and cross one another.`,seed:15,colors:[[.025,.025,.09],[.78,.12,.72],[.08,.64,.96],[.98,.62,.22]],fragment:`
void main() {
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
  vec3 color = u_colorA;
  ...
  gl_FragColor = vec4(color, 1.0);
}`}),S({id:`grainient-field`,name:`Grainient Field`,description:`Broad moving colour masses finished with tactile animated grain.`,seed:27,colors:[[.035,.025,.09],[.9,.2,.42],[.24,.22,.94],[.02,.86,.72]],fragment:`
void main() {
  vec2 uv = gl_FragCoord.xy / u_resolution.xy;
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
...
}`}),S({id:`seeded-gradient-generator`,name:`Seeded Gradient Generator`,description:`One numeric seed generates repeatable bands, blooms and colour layouts.`,seed:6,colors:[[.02,.03,.09],[.46,.12,.94],[.02,.82,.74],[.98,.42,.22]],fragment:`
void main() {
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
  ...
  gl_FragColor = vec4(color, 1.0);
}`})],w={id:`silk`,name:`Silk`,description:`Satin folds with a soft moving sheen.`,category:`gradient`,uniforms:{u_colorA:{type:`color`,value:[.06,.05,.18],label:`Shadow`},u_colorB:{type:`color`,value:[.55,.35,.95],label:`Highlight`},u_speed:{type:`float`,value:.4,min:0,max:2,label:`Speed`},u_scale:{type:`float`,value:1.4,min:.3,max:4,label:`Scale`},u_warp:{type:`float`,value:.7,min:0,max:2,label:`Warp`},u_freq:{type:`float`,value:4,min:1,max:12,label:`Weave`},u_sheen:{type:`float`,value:2.2,min:.5,max:6,label:`Sheen`},u_grain:{type:`float`,value:.03,min:0,max:.2,label:`Grain`}},fragment:`
void main() {
  vec2 uv = gl_FragCoord.xy / u_resolution.xy;
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
...
`},T={id:`spotlight`,name:`Spotlight`,description:`A dark surface your cursor reveals like a torch.`,category:`interactive`,interactive:!0,colorRoles:{u_bg:`dark`,u_glow:`accent`,u_detail:`mid`},uniforms:{u_bg:{type:`color`,value:[.03,.035,.06],label:`Hidden`},u_glow:{type:`color`,value:[1,.72,.35],label:`Light`},u_detail:{type:`color`,value:[.25,.45,.85],label:`Pattern`},u_radius:{type:`float`,value:.55,min:.1,max:1.6,label:`Radius`},u_soft:{type:`float`,value:.6,min:.05,max:1.5,label:`Softness`},u_scale:{type:`float`,value:3,min:.5,max:10,label:`Pattern scale`},u_speed:{type:`float`,value:.3,min:0,max:1.5,label:`Speed`},u_ambient:{type:`float`,value:.18,min:0,max:.6,label:`Ambient`}},fragment:`
void main() {
  vec2 uv = gl_FragCoord.xy / u_resolution.xy;
  vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;
  float t = u_time * u_speed;
...
`};function A(e){return{...e,uniforms:{u_colorA:{type:`color`,value:[.03,.05,.12],label:`Base`},u_colorB:{type:`color`,value:[.35,.18,.95],label:`Color`},u_accent:{type:`color`,value:[.1,.9,.78],label:`Accent`},u_speed:{type:`float`,value:.45,min:0,max:2,label:`Speed`},u_scale:{type:`float`,value:2.2,min:.3,max:8,label:`Scale`},u_intensity:{type:`float`,value:1,min:0,max:2.5,label:`Intensity`}}}}var j=A({id:`grain-gradient`,name:`Grain Gradient`,category:`gradient`,description:`Soft drifting colour with tactile animated grain.`,fragment:`void main(){vec2 uv=gl_FragCoord.xy/u_resolution.xy;vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;vec2 w=vec2(fbm(p*u_scale+t*.12),fbm(p*u_scale+5.0-t*.1));float n=fbm(p*u_scale+w*1.4+t*.08)*.5+.5;vec3 c=mix(u_colorA,u_colorB,smoothstep(.05,.85,n));c=mix(c,u_accent,smoothstep(.58,1.0,n)*.55);c+=grain(uv+fract(t))*.13*u_intensity;gl_FragColor=vec4(c,1.0);}`}),M=A({id:`color-bends`,name:`Color Bends`,category:`gradient`,description:`Broad luminous colour bands folding through space.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float w=fbm(p*u_scale*.6+t*.12);float b=sin((p.x+sin(p.y*1.7+t)*.45+w*.7)*4.0);float k=.5+.5*b;vec3 c=mix(u_colorA,u_colorB,smoothstep(.05,.8,k));c=mix(c,u_accent,pow(smoothstep(.55,1.0,k),2.0));c+=u_accent*pow(max(0.0,b),8.0)*.35*u_intensity;gl_FragColor=vec4(c,1.0);}`}),N=A({id:`ribbon-flow`,name:`Ribbon Flow`,category:`gradient`,description:`Layered twisting ribbons with depth and highlights.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;vec3 c=u_colorA;for(int i=0;i<6;i++){float f=float(i);float y=sin(p.x*(1.2+f*.17)+t*(.5+f*.08)+f)*(.18+f*.025);y+=snoise(vec2(p.x*.7+f,t*.15))*0.16;float r=exp(-abs(p.y-y-f*.08+.2)*18.0);vec3 rc=mix(u_colorB,u_accent,fract(f*.37));c+=rc*r*(.22+.1*f);}c*=u_intensity;gl_FragColor=vec4(c,1.0);}`}),P=A({id:`neuro-noise`,name:`Neuro Noise`,category:`organic`,description:`Branching neural filaments grown from warped noise.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;vec2 q=p*u_scale;float a=fbm(q+vec2(t*.15,0.0));float b=fbm(q*1.7+vec2(-t*.12,4.0));float v=abs(sin((a+b)*12.0));float line=1.0-smoothstep(.04,.18,v);float glow=1.0-smoothstep(.05,.55,v);vec3 c=mix(u_colorA,u_colorB,glow*.35);c+=u_accent*(line+glow*.25)*u_intensity;gl_FragColor=vec4(c,1.0);}`}),F=A({id:`warp-tunnel`,name:`Warp Tunnel`,category:`space`,description:`A luminous radial tunnel rushing toward infinity.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float r=length(p)+.001;float a=atan(p.y,p.x);float n=fbm(vec2(a*1.8,log(r)*u_scale-t));float bands=.5+.5*sin(log(r)*18.0-u_time*3.0+n*5.0);float spokes=.5+.5*sin(a*14.0+n*3.0);float g=pow(bands*spokes,4.0)/(1.0+r*2.0);vec3 c=mix(u_colorA,u_colorB,bands*.35)+u_accent*g*u_intensity;gl_FragColor=vec4(c,1.0);}`}),I=A({id:`spiral-field`,name:`Spiral Field`,category:`geometric`,description:`Hypnotic polar bands curling into a soft centre.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float r=length(p);float a=atan(p.y,p.x);float s=.5+.5*sin(a*7.0-log(r+.08)*u_scale*5.0-t*3.0+fbm(p*2.0)*2.0);float edge=smoothstep(.35,.65,s);vec3 c=mix(u_colorA,u_colorB,edge);c+=u_accent*pow(s,7.0)*exp(-r*.7)*u_intensity;gl_FragColor=vec4(c,1.0);}`}),L=A({id:`dot-orbit`,name:`Dot Orbit`,category:`interactive`,interactive:!0,description:`Glowing particles orbit moving cursor-influenced attractors.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;vec2 m=mouseSmoothPos();vec3 c=u_colorA;for(int i=0;i<9;i++){float f=float(i);float a=t*(.45+f*.025)+f*TAU/9.0;float r=.22+.055*f;vec2 o=vec2(cos(a),sin(a))*r+sin(f*2.3+t)*.08;o=mix(o,m*.25,u_mouseEnter);float d=length(p-o);float g=.012/(d*d+.004);c+=mix(u_colorB,u_accent,fract(f*.31))*g*.035*u_intensity;}gl_FragColor=vec4(c,1.0);}`}),R=A({id:`smoke-ring`,name:`Smoke Ring`,category:`organic`,description:`Turbulent expanding rings with a coloured smoky glow.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float r=length(p);float n=fbm(p*u_scale+vec2(t*.15,-t*.1));float sum=0.0;for(int i=0;i<4;i++){float age=fract(t*.18+float(i)*.25);float ring=exp(-abs(r-(.15+age*1.15)+n*.12)*22.0)*(1.0-age);sum+=ring;}vec3 c=mix(u_colorA,u_colorB,clamp(sum*.45,0.0,1.0));c+=u_accent*sum*.65*u_intensity;gl_FragColor=vec4(c,1.0);}`}),ce=A({id:`color-panels`,name:`Color Panels`,category:`gradient`,description:`Sliding geometric panels with softened colour seams.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float x=p.x+sin(p.y*1.5+t)*.28+fbm(p*.8)*.2;float panel=floor((x*u_scale+t*.25)+3.0);float local=fract(x*u_scale+t*.25);vec3 pc=mix(u_colorB,u_accent,fract(panel*.618));float seam=smoothstep(.02,.18,local)*smoothstep(.02,.18,1.0-local);vec3 c=mix(u_colorA,pc,seam*u_intensity);gl_FragColor=vec4(c,1.0);}`}),z=A({id:`line-waves`,name:`Line Waves`,category:`geometric`,description:`Layered glowing sine lines flowing with parallax.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;vec3 c=u_colorA;for(int i=0;i<12;i++){float f=float(i);float y=-.8+f*.14+sin(p.x*u_scale+f*.5+t*(.5+f*.02))*.09;float d=abs(p.y-y);float l=1.0-smoothstep(.008,.025,d);float g=exp(-d*35.0);c+=mix(u_colorB,u_accent,f/11.0)*(l+g*.15)*u_intensity;}gl_FragColor=vec4(c,1.0);}`}),B=A({id:`light-beams`,name:`Light Beams`,category:`space`,description:`Soft volumetric beams cutting through animated haze.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;vec2 source=vec2(sin(t*.3)*.35,1.1);vec2 d=p-source;float a=atan(d.y,d.x);float haze=fbm(p*u_scale+vec2(t*.1,0.0))*.5+.5;float beams=pow(.5+.5*sin(a*18.0+haze*4.0),10.0);float fall=1.0/(1.0+dot(d,d));vec3 c=u_colorA+u_colorB*haze*.14;c+=u_accent*beams*fall*haze*u_intensity;gl_FragColor=vec4(c,1.0);}`}),V=A({id:`magic-rings`,name:`Magic Rings`,category:`interactive`,interactive:!0,description:`Nested luminous rings bending around the pointer.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;vec2 m=mouseSmoothPos();p-=m*.28*u_mouseEnter;float t=u_time*u_speed;float r=length(p)+fbm(p*u_scale+t*.08)*.08;float rings=.5+.5*sin(r*32.0-t*4.0);float line=pow(rings,12.0);vec3 c=mix(u_colorA,u_colorB,rings*.22);c+=u_accent*line*exp(-r*.7)*u_intensity;gl_FragColor=vec4(c,1.0);}`}),H=A({id:`network-field`,name:`Network Field`,category:`geometric`,description:`A drifting constellation connected by luminous threads.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y*u_scale;vec2 id=floor(p),gv=fract(p)-.5;float t=u_time*u_speed;float glow=0.0;for(int j=-1;j<=1;j++)for(int i=-1;i<=1;i++){vec2 n=vec2(float(i),float(j));vec2 o=hash22(id+n)-.5;o+=sin(t+hash22(id+n)*TAU)*.18;vec2 q=n+o-gv;float d=length(q);glow+=.015/(d*d+.01);}float links=1.0-smoothstep(.03,.08,abs(gv.x-gv.y+sin(id.x+id.y+t)*.15));vec3 c=u_colorA+u_colorB*glow*.18+pow(max(glow,0.0),1.0/3.0)*u_accent*.02;c+=u_accent*links*.18*u_intensity;gl_FragColor=vec4(c,1.0);}`}),U=A({id:`branching-trunk`,name:`Branching Trunk`,category:`organic`,description:`Electrical tree-like tendrils branching through darkness.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float y=p.y;float x=p.x;float trunk=abs(x-sin(y*u_scale+t)*.08);float b1=abs(x-sin(y*3.0+t)*(.12+.25*smoothstep(-.2,.8,y)));float b2=abs(x+sin(y*3.7-t*.8)*(.1+.3*smoothstep(-.1,.9,y)));float d=min(trunk,min(b1,b2));float g=exp(-d*45.0)*(1.0-smoothstep(.85,1.25,abs(y)));vec3 c=u_colorA+mix(u_colorB,u_accent,smoothstep(-.7,.8,y))*g*u_intensity;gl_FragColor=vec4(c,1.0);}`}),W=A({id:`cloudscape`,name:`Cloudscape`,category:`space`,description:`Layered illuminated clouds drifting across a deep sky.`,fragment:`void main(){vec2 uv=gl_FragCoord.xy/u_resolution.xy;vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float n=fbm(vec3(p*u_scale+vec2(t*.1,0.0),t*.08))*.5+.5;float clouds=smoothstep(.38,.72,n+uv.y*.12);float edge=smoothstep(.3,.65,n)-smoothstep(.62,.9,n);vec3 c=mix(u_colorA,u_colorB,clouds*.8);c+=u_accent*edge*.32*u_intensity;c*=.72+.4*uv.y;gl_FragColor=vec4(c,1.0);}`}),G=A({id:`ocean-surface`,name:`Ocean Surface`,category:`organic`,description:`Perspective rolling waves with a bright reflective horizon.`,fragment:`void main(){vec2 uv=gl_FragCoord.xy/u_resolution.xy;vec2 p=uv*2.0-1.0;p.x*=u_resolution.x/u_resolution.y;float t=u_time*u_speed;float horizon=.18;float z=1.0/max(.06,uv.y-horizon);vec2 q=vec2(p.x*z,z)*u_scale*.35;float w=sin(q.x*2.0+t)+sin(q.y*1.3-t*.8)+snoise(q*.7+t*.2);float crest=pow(.5+.5*sin(w*2.0),8.0);float sky=smoothstep(horizon-.02,horizon+.02,uv.y);vec3 sea=mix(u_colorA,u_colorB,.35+.35*w);vec3 c=mix(u_colorA*.5,sea,sky);c+=u_accent*crest*sky*u_intensity/(1.0+z*.06);gl_FragColor=vec4(c,1.0);}`}),le=A({id:`liquid-shape-distortion`,name:`Liquid Shapes`,category:`organic`,description:`Psychedelic liquid forms with bloom and soft shadows.`,fragment:`void main(){vec2 uv=gl_FragCoord.xy/u_resolution.xy;vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;vec2 w=vec2(fbm(p*u_scale+t*.12),fbm(p*u_scale+4.0-t*.1));float f=fbm(p*u_scale+w*1.8);float shape=smoothstep(-.15,.2,f);float rim=smoothstep(.02,.22,abs(f-.03));vec3 c=mix(u_colorA,u_colorB,shape);c+=u_accent*(1.0-rim)*u_intensity;c+=grain(uv+fract(t))*.04;gl_FragColor=vec4(c,1.0);}`}),ue=A({id:`stripe-flow`,name:`Stripe Flow`,category:`gradient`,description:`Warped Stripe-style colour bands in constant motion.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float w=fbm(p*u_scale*.7+vec2(t*.12,0.0));float s=.5+.5*sin((p.x+p.y*.25+w*.8)*u_scale*5.0+t);vec3 c=mix(u_colorA,u_colorB,smoothstep(.12,.72,s));c=mix(c,u_accent,smoothstep(.7,1.0,s)*u_intensity);gl_FragColor=vec4(c,1.0);}`}),de=A({id:`conic-flow`,name:`Conic Flow`,category:`gradient`,description:`Rotating conic colour wedges softened by procedural noise.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float a=atan(p.y,p.x)/TAU+.5;float n=fbm(p*u_scale+t*.08)*.12;float wd=max(1.0,floor(u_scale));float s=fract(a*wd+n+t*.08);float sm=min(s,1.0-s)*2.0;vec3 c=mix(u_colorA,u_colorB,smoothstep(.05,.85,sm));c=mix(c,u_accent,smoothstep(.55,.95,sm)*u_intensity);c*=1.1-.25*length(p);gl_FragColor=vec4(c,1.0);}`}),fe=A({id:`smoke-gradient`,name:`Smoke Gradient`,category:`gradient`,description:`A coloured gradient transported through slow smoke.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;vec2 q=p*u_scale*.6;vec2 w=vec2(fbm(q+t*.1),fbm(q+6.0-t*.08));float n=fbm(q+w*2.0+t*.05)*.5+.5;float plume=smoothstep(.18,.82,n);vec3 c=mix(u_colorA,u_colorB,plume);c+=u_accent*pow(plume,3.0)*.35*u_intensity;gl_FragColor=vec4(c,1.0);}`}),pe=A({id:`radar-sweep`,name:`Radar Sweep`,category:`interactive`,interactive:!0,description:`A scanning radial grid with cursor-positioned echoes.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float r=length(p),a=atan(p.y,p.x);float circles=1.0-smoothstep(.02,.045,abs(fract(r*u_scale*2.0)-.5));float spokes=1.0-smoothstep(.015,.04,abs(sin(a*6.0)));float sweep=pow(max(0.0,cos(a-t*2.0)),22.0);float echo=exp(-length(p-mousePos())*8.0)*u_mouseEnter;vec3 c=u_colorA+u_colorB*(circles+spokes)*.18;c+=u_accent*(sweep*(1.0-r)+echo)*u_intensity;gl_FragColor=vec4(c,1.0);}`}),me=A({id:`pixel-trail`,name:`Pixel Trail`,category:`interactive`,interactive:!0,description:`A pixelated glow trail that follows movement and clicks.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;vec2 cell=floor(p*u_scale*18.0)/(u_scale*18.0);float d=length(cell-mousePos());float trail=exp(-d*7.0)*u_mouseEnter;trail+=abs(rippleField(cell,1.0,18.0,1.2));float flick=hash21(cell*31.0+floor(u_time*8.0));float v=smoothstep(.1,.9,trail*(.6+.7*flick));vec3 c=mix(u_colorA,u_colorB,v);c+=u_accent*v*u_intensity;gl_FragColor=vec4(c,1.0);}`}),he=A({id:`antigravity-field`,name:`Antigravity Field`,category:`interactive`,interactive:!0,description:`Floating particles pushed away by the pointer.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y*u_scale;vec2 m=mousePos()*u_scale;vec2 id=floor(p),gv=fract(p)-.5;float t=u_time*u_speed;float stars=0.0;for(int j=-1;j<=1;j++)for(int i=-1;i<=1;i++){vec2 n=vec2(float(i),float(j));vec2 o=hash22(id+n)-.5;o+=sin(t+hash22(id+n)*TAU)*.22;vec2 wp=id+n+o;vec2 away=normalize(wp-m+vec2(.001))*exp(-length(wp-m)*.7)*u_mouseEnter;float d=length(n+o+away*.65-gv);stars+=.012/(d*d+.008);}vec3 c=u_colorA+mix(u_colorB,u_accent,clamp(stars*.08,0.0,1.0))*stars*.08*u_intensity;gl_FragColor=vec4(c,1.0);}`}),ge=A({id:`metallic-paint`,name:`Metallic Paint`,category:`iridescent`,description:`Flowing chrome bands with glossy coloured reflections.`,fragment:`float metalH(vec2 p,float t){return fbm(p*u_scale+vec2(t*.12,-t*.08));}void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float e=.012;float h=metalH(p,t);vec2 g=vec2(metalH(p+vec2(e,0),t)-h,metalH(p+vec2(0,e),t)-h)/e;vec3 n=normalize(vec3(-g*.35,1.0));float ref=.5+.5*n.x;float spec=pow(max(0.0,dot(n,normalize(vec3(.4,.6,1.0)))),28.0);vec3 c=mix(u_colorA,u_colorB,ref);c=mix(c,u_accent,.5+.5*sin(h*8.0+t));c+=spec*u_intensity;gl_FragColor=vec4(c,1.0);}`}),_e=A({id:`shifting-sands`,name:`Shifting Sands`,category:`organic`,description:`Wind-sculpted dune ridges with travelling highlights.`,fragment:`float dune(vec2 p,float t){float n=fbm(p*u_scale+vec2(t*.08,0.0));return 1.0-abs(n);}void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float e=.012;float h=dune(p,t);vec2 g=vec2(dune(p+vec2(e,0),t)-h,dune(p+vec2(0,e),t)-h)/e;vec3 n=normalize(vec3(-g*.22,1.0));float light=clamp(dot(n,normalize(vec3(-.5,.5,1.0))),0.0,1.0);vec3 c=mix(u_colorA,u_colorB,h*.7);c+=u_accent*pow(light,3.0)*u_intensity;gl_FragColor=vec4(c,1.0);}`}),ve=A({id:`cosmic-vortex`,name:`Cosmic Vortex`,category:`interactive`,interactive:!0,description:`Spiralling energy around a cursor-controlled centre.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;vec2 centre=mouseSmoothPos()*.45*u_mouseEnter;p-=centre;float t=u_time*u_speed;float r=length(p)+.01,a=atan(p.y,p.x);float n=fbm(vec2(a*2.0,r*u_scale*3.0-t));float arms=.5+.5*sin(a*7.0-r*12.0+t*2.0+n*4.0);float glow=pow(arms,6.0)*exp(-r*.9)+.025/r;vec3 c=mix(u_colorA,u_colorB,arms*.25);c+=u_accent*glow*u_intensity;gl_FragColor=vec4(c,1.0);}`}),ye=A({id:`smoke-ink`,name:`Smoke Ink`,category:`organic`,description:`Dense ink tendrils curling through coloured haze.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;vec2 q=p*u_scale;vec2 w=vec2(fbm(q+t*.08),fbm(q+9.0-t*.06));float n=fbm(q+w*2.3);float ink=smoothstep(-.3,.45,n);float vein=pow(1.0-abs(sin(n*9.0)),5.0);vec3 c=mix(u_colorA,u_colorB,ink*.75);c+=u_accent*vein*ink*.55*u_intensity;gl_FragColor=vec4(c,1.0);}`}),be=A({id:`moire-interference`,name:`Moiré Interference`,category:`geometric`,description:`Overlapping line fields producing shifting interference.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float a=sin(length(p-vec2(sin(t)*.2,0.0))*u_scale*30.0);float b=sin(length(p+vec2(cos(t*.8)*.25,0.0))*u_scale*30.5);float m=.5+.5*a*b;float lines=pow(m,5.0);vec3 c=mix(u_colorA,u_colorB,m*.45);c+=u_accent*lines*u_intensity;gl_FragColor=vec4(c,1.0);}`}),K=A({id:`reaction-diffusion`,name:`Reaction Diffusion`,category:`organic`,description:`Evolving coral and zebra-like cellular bands.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;vec2 q=p*u_scale;float n=fbm(q+t*.05);float cells=sin(q.x*3.0+n*8.0)+sin(q.y*3.2-n*7.0)+snoise(vec3(q*1.8,t*.15))*1.5;float band=1.0-smoothstep(.12,.5,abs(sin(cells*1.6)));vec3 c=mix(u_colorA,u_colorB,smoothstep(-1.0,1.0,cells)*.65);c+=u_accent*band*u_intensity;gl_FragColor=vec4(c,1.0);}`}),xe=A({id:`kaleidoscope`,name:`Kaleidoscope`,category:`geometric`,description:`Mirrored procedural colour with crystalline symmetry.`,fragment:`void main(){vec2 p=(gl_FragCoord.xy*2.0-u_resolution.xy)/u_resolution.y;float t=u_time*u_speed;float r=length(p),a=atan(p.y,p.x);float seg=TAU/8.0;a=abs(mod(a+t*.08,seg)-seg*.5);vec2 q=vec2(cos(a),sin(a))*r*u_scale;float n=fbm(q+vec2(t*.12,-t*.1))*.5+.5;float facets=.5+.5*sin(n*12.0+r*15.0);vec3 c=mix(u_colorA,u_colorB,n);c=mix(c,u_accent,pow(facets,4.0)*u_intensity);gl_FragColor=vec4(c,1.0);}`}),q=[j,M,N,ce,ue,de,fe],J=[P,R,U,G,le,_e,ye,K],Y=[L,V,pe,me,he,ve],X=[F,B,W],Z=[I,z,H,be,xe],Q=[ge];[...q,...Q,...Y,...J,...X,...Z];var $=[p,te,w,...q,..._,...C,oe,y,x,...Q,d,f,T,re,...Y,m,b,k,ne,se,...J,E,v,ie,...X,D,O,ae,...Z],Se=[{id:`gradient`,label:`Gradient`},{id:`iridescent`,label:`Iridescent`},{id:`interactive`,label:`Interactive`},{id:`organic`,label:`Organic`},{id:`space`,label:`Space`},{id:`geometric`,label:`Geometric`}];Object.fromEntries($.map(e=>[we(e.id),e]));function Ce(e){return $.find(t=>t.id===e)}function we(e){return e.replace(/-([a-z])/g,(e,t)=>t.toUpperCase())}export{o as a,r as c,ee as i,s as l,Ce as n,n as o,$ as r,a as s,Se as t,e as u};