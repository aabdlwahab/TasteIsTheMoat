...
`},oe={id:`holo-foil`,name:`Iridescent Foil`,description:`Crinkled foil under real spectral thin-film interference.`,category:`iridescent`,interactive:!0,uniforms:{u_thickness:{type:`float`,value:520,min:150,max:1200,label:`Film nm`},u_ior:{type:`float`,value:1.62,min:1.1,max:2.8,label:`Film IOR`},u_scale:{type:`float`,value:2.6,min:.6,max:8,label:`Crinkle scale`},u_relief:{type:`float`,value:.28,min:.02,max:1,label:`Relief`},u_lightHeight:{type:`float`,value:.9,min:0,max:3,label:`Light height`},u_gloss:{type:`float`,value:.55,min:0,max:1,label:`Gloss`},u_gain:{type:`float`,value:1.3,min:0,max:3,label:`Gain`},u_grain:{type:`float`,value:.035,min:0,max:.2,label:`Grain`}},fragment:`
...
void main() {
  ...
  vec3 col = vec3(0.022, 0.024, 0.030);
  col += irid * (0.20 + 1.1 * spec + 0.45 * fres) * (0.35 + 0.8 * diff) * u_gain;
  col += vec3(1.0, 0.97, 0.93) * spec * 0.35;
  col *= 0.75 + 0.45 * h0;
